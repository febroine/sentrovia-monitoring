...
export function ReportComparison({ report, showCoverage = true, compactWhenNoPrior = false }: { report: GeneratedReport; showCoverage?: boolean; compactWhenNoPrior?: boolean }) {
  ...
  const noPriorChecks = compactWhenNoPrior && comparison.previousCompletedChecks === 0;
  return (
    <section aria-label="Previous period and reference budget" className="border-t border-border pt-4">
      <h3 className="text-sm font-medium">{noPriorChecks ? "Previous period" : "Compared with previous period"}</h3>
      ...
      {noPriorChecks ? (
        <div className="mt-3 text-sm">
          <p>No previous checks to compare. {values.referenceLabel} reference budget: <span className="font-medium tabular-nums">{values.budget}</span></p>
          <p className="mt-1 text-xs text-muted-foreground">{values.budgetDetail}</p>
        </div>
      ) : <dl className="mt-3 grid gap-3 sm:grid-cols-3">
        <div>
          <dt className="text-xs text-muted-foreground">Check success change</dt>
      ...
      {showCoverage && coverage ? <p className="mt-3 border-t border-border/60 pt-3 text-xs text-muted-foreground">
